...
move_uploaded_file($PmainImage,"img/". basename($_FILES["PmainImage"]["name"]));
$PmI="img/" . basename($_FILES["PmainImage"]["name"]); //basename 從一個完整的路徑中提取出文件名，而不包括路徑。

move_uploaded_file($subgraph1,"img/". $_FILES["subgraph1"]['name']);
$s1="img/" . $_FILES["subgraph1"]["name"];

move_uploaded_file($subgraph2,"img/". $_FILES["subgraph2"]['name']);
$s2="img/" . $_FILES["subgraph2"]["name"];

move_uploaded_file($subgraph3,"img/". $_FILES["subgraph3"]['name']);
$s3="img/" . $_FILES["subgraph3"]["name"];

move_uploaded_file($subgraph4,"img/". $_FILES["subgraph4"]['name']);
$s4="img/" . $_FILES["subgraph4"]["name"];
...
$stmt ->bind_param('sssssssssssssss',$Pname,$Psort,$PmI,$s1,$s2,$s3,$s4,$Pprice,$Pdiscount,$PfinalPrice,$Pstorage,$Pstatus,$Pintroduction,$Pstandard,$editor);
